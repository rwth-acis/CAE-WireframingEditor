...
function FrontendComponentMapper(graph){
    ...
    var attrTpl = '{"id":"<%=id%>[<%=attrName%>]", "name":"<%=attrName%>", "value":{"id":"<%=id%>[<%=attrName%>]", "name":"<%=attrName%>", "value":<%=value%>}}';
    ...
    for(var i=0; i < model.getMeta().attributes.length; i++){
        ...
        var json = attrCompiled({id : widgetNodeId, attrName : attrName, value: '"' + attr.value + '"'});
        if(attrName === 'name')
            label = json;
        var id = Util.GUID();
    ...
    for(var i=0;i<parent.children.length;i++){
        ...
        var label = attrCompiled({id: cell.id, attrName: 'label', value: '"' + cell.value.getAttribute('label') + '"'});
        var attributes = {};
        var id = Util.GUID();
        attributes[id] = JSON.parse(attrCompiled({id: cell.id, attrName : 'type', value : '"'+cell.constructor.HTML_NODE_NAME + '"'}));
        id = Util.GUID();
        attributes[id] = JSON.parse(attrCompiled({id: cell.id, attrName : 'id', value :'""'}));
        id = Util.GUID();
        attributes[id] = JSON.parse(attrCompiled({id: cell.id, attrName : 'static', value: false}));
        id = Util.GUID();
        attributes[id] = JSON.parse(attrCompiled({id: cell.id, attrName : 'collaborative', value: false}));
        ...
        var edgeLabel = attrCompiled({id: edgeId, attrName: 'label', value: '""'});
